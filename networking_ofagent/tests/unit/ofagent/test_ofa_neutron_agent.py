...
class TestOFANeutronAgent(ofa_test_base.OFAAgentTestBase):
    ...
    def test__check_port_status_list_no_ofport_change(self):
        port_status_list = [_make_portstatus('tapd3315981-0b', 'add'),
                            _make_portstatus('tapd3315982-0b', 'add'),
                            _make_portstatus('tapd3315983-0b', 'add')]
        port_info = {}
        check_ports = self.agent._check_port_status_list(port_status_list,
                                                         port_info)
        self.assertFalse(check_ports)
        self.assertFalse(port_info)

    def test__check_port_status_list_ofport_change(self):
        name = 'tapd3315981-0b'
        port_status_list = [_make_portstatus(name, 'del'),
                            _make_portstatus('tapd3315982-0b', 'add'),
                            _make_portstatus(name, 'add')]
        port_info = {}
        ps_mod = port_status_list[0]._replace(reason='mod')
        expected_check_ports = {name: ps_mod}
        expected_port_info = {'updated': set([name])}
        check_ports = self.agent._check_port_status_list(port_status_list,
                                                         port_info)
        self.assertEqual(check_ports, expected_check_ports)
        self.assertEqual(port_info, expected_port_info)

    def test__check_port_status_list_exist_as_removed_port(self):
        name = 'tapd3315981-0b'
        port_status_list = [_make_portstatus(name, 'del'),
                            _make_portstatus('tapd3315982-0b', 'add'),
                            _make_portstatus(name, 'add')]
        port_info = {'removed': set([name])}
        expected_port_info = port_info.copy()
        check_ports = self.agent._check_port_status_list(port_status_list,
                                                         port_info)
        self.assertFalse(check_ports)
        self.assertEqual(port_info, expected_port_info)

    def test__check_port_status_list_ofport_changes(self):
        name1 = 'tapd3315981-0b'
        name2 = 'tapd3315982-0b'
        name3 = 'tapd3315983-0b'
        port_status_list = [_make_portstatus(name1, 'del'),
                            _make_portstatus(name2, 'del'),
                            _make_portstatus(name1, 'add'),
                            _make_portstatus(name3, 'del'),
                            _make_portstatus('tapd3315982-00', 'add'),
                            _make_portstatus(name3, 'add')]
        port_info = {'updated': set(['tapd3315981-0c', 'tapd3315982-0c'])}
        ps_mod1 = port_status_list[0]._replace(reason='mod')
        ps_mod3 = port_status_list[3]._replace(reason='mod')
        expected_check_ports = {name1: ps_mod1, name3: ps_mod3}
        expected_port_info = port_info.copy()
        expected_port_info['updated'] |= set([name1, name3])
        check_ports = self.agent._check_port_status_list(port_status_list,
                                                         port_info)
        self.assertEqual(check_ports, expected_check_ports)
        self.assertEqual(port_info, expected_port_info)
