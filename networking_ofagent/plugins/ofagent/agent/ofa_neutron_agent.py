...
class OFANeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def _check_port_status_list(self, port_status_list, port_info):
        ...
        LOG.debug("Agent daemon_loop check_ports: %(checkp)s "
                  "check_ports_set: %(checkset)s "
                  "check_ports_all: %(checkpall)s",
                  {'checkp': check_ports,
                   'checkset': check_ports_set,
                   'checkpall': check_ports_all})
        if check_ports_set:
